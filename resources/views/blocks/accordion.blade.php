...
<section data-gsap-anim="section" @if($sectionId) id="{{ $sectionId }}" @endif class="accordion faq relative overflow-hidden -smt {{ $block->classes }} {{ $customClass }} {{ $sectionClass }}">
	...
	<div class="__wrapper c-main relative z-2">
		...
		<div class="grid grid-cols-1 md:grid-cols-[1.3fr_2fr] gap-20 mt-10">
			@if (!empty($g_accordion['image']))
			<img data-gsap-element="image" class="object-cover __img order1 img-xl" src="{{ $g_accordion['image']['url'] }}" alt="{{ $g_accordion['image']['alt'] ?? '' }}">
			@endif
	...
	@if ($bgimage)
	<svg class="__bg--first absolute -top-20 -left-50 z-1" xmlns="http://www.w3.org/2000/svg" width="772" height="1648" viewBox="0 0 772 1648" fill="none">
